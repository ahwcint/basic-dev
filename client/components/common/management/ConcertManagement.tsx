...
function ConcertManagement({ className, ...prop }: ComponentProps<typeof BaseContainer>) {
  ...
  useEffect(() => {
    ...
    if (currentTab && Object.values(TabsContentValue).includes(currentTab as TabsContentValue)) {
    ...
    <BaseContainer className={cn('px-0', className)} {...prop}>
      <Tabs value={tab} className="h-full">
        <ScrollArea className="h-full py-2" scrollHideDelay={500}>
          ...
          <TabsContent value={TabsContentValue.OVERVIEW} className="gap-5 flex flex-col py-4">
            <ConcertInformation />
          </TabsContent>
 ...
 * @access ADMIN
 */
export function ConcertManagementWrapper(props: ComponentProps<typeof ConcertManagement>) {
  return (
